...
import authApi from '@/admin/apis/auth'
import websocketManager from '@/admin/services/websocket-manager'
import { setCSRFToken } from '@/admin/utils/requester'
...
export const useAuthStore = defineStore('auth', () => {
  ...
  const syncCSRFToken = async (): Promise<void> => {
    const response = await authApi.csrfToken()
    if (response.status && response.data?.csrf_token) {
